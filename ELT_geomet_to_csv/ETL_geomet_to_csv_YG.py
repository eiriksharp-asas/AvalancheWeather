...
for station in stations:
    ...
    for layer in layers:
        ...
        if layer in ['GDPS.ETA_PN-SLP', 'GDPS.ETA_HR']:
            fx['date'] = [date_time.date() for date_time in fx['time']]
            fx_table[layer+'_mean']=fx.groupby([fx.date])['value'].mean().round(0)
        if layer in ['GDPS.ETA_RN']:
            fx['date'] = [date_time.date() for date_time in fx['time']]
            fx_table[layer + '_total'] = fx.groupby([fx.date])['value'].sum().round(0)
        if layer in [ 'GDPS.ETA_SN']:
            fx['date'] = [date_time.date() for date_time in fx['time']]
            fx_table[layer + '_5%'] = (fx.groupby([fx.date])['value'].sum()/50).round(0)
            fx_table[layer + '_10%'] = (fx.groupby([fx.date])['value'].sum()/100).round(0)        
        if layer in ['GDPS.PRES_WD.800.3h']:
            fx['date'] = [date_time.date() for date_time in fx['time']]
        ...
        if layer in ['GDPS.PRES_WSPD.800.3h']:
            fx['date']=[date_time.date() for date_time in fx['time']]
            fx['value']=fx['value'].apply(lambda x: ms_to_windspeed(x))
            fx_table[layer]=fx.groupby([fx.date])['value'].agg(pd.Series.mode).to_frame()
        if layer in ['GEPS.DIAG.24_PRMM.ERGE1', 'GEPS.DIAG.24_RNMM.ERGE1', 'GEPS.DIAG.24_RNMM.ERGE10', 'GEPS.DIAG.24_RNMM.ERGE25',
            ...
            fx_table[layer] = fx.at_time('08:00').round(0)
